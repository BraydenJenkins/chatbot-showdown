...
public class RolesManager : NetworkBehaviour
{
    ...
    public void SendRoleQuestions()
    {
        ...
        for (int i = 0; i < players.Count; i++)
        {
            ...
            // reset myTurn just in case
            player.myTurn.Value = false;
            player.currentAvatarIndex.Value = -1;
    ...
    private void CompileResponses()
    {
        ...
        List<(string, ulong)> allWords = new List<(string, ulong)>();
        foreach (var response in allRoleResponses)
        {
            string[] words = response.Item1.Split(' ');
            foreach (var word in words)
            {
                allWords.Add((word, response.Item2));
            }
        }
        foreach (var response in allAdjectiveResponses)
        {
            string[] words = response.Item1.Split(' ');
            foreach (var word in words)
            {
        ...
        foreach (var response in allFreeResponses)
        {
            string[] words = response.Item1.Split(' ');
            foreach (var word in words)
            {
        ...
        // remove all empty strings and default options from allWords
        allWords.RemoveAll(word => word.Item1 == "");
        allWords.RemoveAll(word => defaultOptions.Contains(word.Item1));

        string allWordsString = "";
        foreach (var word in allWords)
        {
            allWordsString += word.Item1 + ", ";
        }
        Debug.Log("[Roles]: All words: " + allWordsString);
        ...
        for (int i = 0; i < players.Count; i++)
        {
            ...
            // I think the above is WAY overcomplicated
            // lets just chop up all words from all responses and send them out randomly,
            // but not to the player who submitted them

            Shuffle(allWords);

            // get the first 20 words that are not from the player

            List<string> chosenResponses = new List<string>();

            for (int j = 0; j < allWords.Count; j++)
            {
                if (allWords[j].Item2 != player.OwnerClientId)
                {
                    chosenResponses.Add(allWords[j].Item1);
                    if (chosenResponses.Count >= 20)
                    {
                        break;
                    }
                }
            ...
            // add some examples too to fill in the gaps and give people enough to work with
            // let's do one example from each category
            int randomIndex = Random.Range(0, exampleRoleAnswers.answers.Count);
            chosenResponses.Add(exampleRoleAnswers.answers[randomIndex]);
            randomIndex = Random.Range(0, exampleAdjectiveAnswers.answers.Count);
            chosenResponses.Add(exampleAdjectiveAnswers.answers[randomIndex]);
            randomIndex = Random.Range(0, exampleFreeAnswers.answers.Count);
            chosenResponses.Add(exampleFreeAnswers.answers[randomIndex]);

            // send the responses to the player
            chosenResponses.AddRange(defaultOptions);
            // using roleOptions as the sole source of responses now
            ...
            Debug.Log("[Roles]: adding bot created listener for " + player.OwnerClientId);
    ...
    private void OnBotCreated(FixedString512Bytes prev, FixedString512Bytes current, ulong clientId)
    {
        ...
        Debug.Log("[Roles]: removing bot created listener for " + clientId);
        player.bot.OnValueChanged -= (prev, current) =>
        {
    ...
    private void CheckReceivedAllBots()
    {
        ...
        currentState = RolesState.Activity;
    ...
    private IEnumerator RunActivity(RolesActivity activity)
    {
        ...
        if (IsServer)
        {
            ...
            Debug.Log("[Roles]: Starting with " + currentPlayer.playerName.Value + "'s turn");
    ...
    public void AdvanceConversationRpc(ulong SenderClientId)
    {
        ...
        if (currentMessageIndex > playerConversations[SenderClientId].messages.Length)
        {
            ...
            Debug.Log("[Roles]: Moving to " + currentPlayer.playerName.Value + "'s turn");
    ...
    private void GoToVoting()
    {
        ...
        currentState = RolesState.Voting;
    ...
    private void OnPlayerVoted(ulong previousValue, ulong newValue, ulong voterId)
    {
        ...
        player.votedPlayer.OnValueChanged -= (prev, current) =>
        {
            OnPlayerVoted(prev, current, voterId);
        };
    ...
    private void CheckAllPlayersVoted()
    {
        ...
        if (currentState != RolesState.Voting)
        {
            return;
        }
        ...
        foreach (var player in players)
        {
            if (voteCounts.ContainsKey(player.OwnerClientId))
            {
                ...
                Debug.Log("[Roles]: " + player.playerName.Value + " received " + player.votes.Value + " votes this round.");
            }
            else
            {
            ...
            Debug.Log("[Roles]: " + player.playerName.Value + " now has " + player.score.Value + " points.");
        }
        ...
        currentState = RolesState.Scoring;
        ...
        if (IsServer)
        {
            StartCoroutine(ShowNextRoundButtonAfterDelay(6));
        }

    }

    private IEnumerator ShowNextRoundButtonAfterDelay(float delay)
    {
        yield return new WaitForSeconds(delay);

        nextRoundButton.interactable = true;
        nextRoundButton.blocksRaycasts = true;
        nextRoundButton.DOFade(1, 0.25f);
    }
    ...
    private async Task<ConversationAPI.Conversation> GetConversation(string playerChatbotPersonality, string goal, string cpuRole, string cpuConversationStarter, int maxAttempts = 3)
    {
        ...
        string outputFormat = "The output must be JSON that describes an array of messages. Each message has a role, content, and animation. Animations are chosen from the following list: ['annoyed shake', 'defeat', 'nod yes', 'pointing', 'salute', 'shake fist', 'shake head no', 'strong taunt', 'mean taunt', 'wave', 'whatever', 'idle']. Please do not use an animation not in the list. Use the following schema: {\\\"messages\\\": [{\\\"role\\\": \\\"<CPU ROLE>\\\", \\\"content\\\": \\\"<CPU's message>\\\", \\\"animation\\\": \\\"<CPU's animation>\\\"}, {\\\"role\\\": \\\"player\\\", \\\"content\\\": \\\"<player's message>\\\", \\\"animation\\\": \\\"<player's animation>\\\"}]}\nEnsure the output JSON is correctly formatted and includes appropriate roles, content, and animations for each message. Begin your message with \\\"{\\\"messages\\\": [\\\" to start the JSON object and end with \\\"]}\\\" to close the JSON object. You do not need to include the word json in your response. You MUST output a valid JSON string (NOT AN OBJECT).";

        string prompt = $"{mainPrompt}\n\n{sceneInformation}\n\n{outputFormat}";

        string fallbackConversation = "{\"messages\":[{\"role\":\"agent\",\"content\":\"Hi there! Welcome to our coffee shop. How can I assist you today?\",\"animation\":\"wave\"},{\"role\":\"player\",\"content\":\"Hello! I was hoping to have a charming conversation to get a free coffee.\",\"animation\":\"pointing\"},{\"role\":\"agent\",\"content\":\"Oh no! It looks like our conversation generator is on a coffee break. Technical difficulties, you know?\",\"animation\":\"shake head no\"},{\"role\":\"player\",\"content\":\"Oh dear, even computers need a caffeine fix sometimes! How about I come back later?\",\"animation\":\"salute\"},{\"role\":\"agent\",\"content\":\"That sounds like a plan! In the meantime, enjoy a virtual coffee on us. Cheers!\",\"animation\":\"wave\"}]}";
        ...
        for (int attempts = 0; attempts < maxAttempts; attempts++)
        {
            try
            {
                ...
                {
                    string exampleConversation = "{\"messages\":[{\"role\":\"agent\",\"content\":\"Hi!\",\"animation\":\"wave\"},{\"role\":\"player\",\"content\":\"Hello there!\",\"animation\":\"wave\"},{\"role\":\"agent\",\"content\":\"How can I assist you today?\",\"animation\":\"salute\"},{\"role\":\"player\",\"content\":\"I need help with my account.\",\"animation\":\"whatever\"},{\"role\":\"agent\",\"content\":\"Sure, I can help with that. What seems to be the issue?\",\"animation\":\"strong taunt\"},{\"role\":\"player\",\"content\":\"I forgot my password.\",\"animation\":\"defeat\"}]}";
